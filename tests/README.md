...
```bash
uv run --extra dev pytest -q                          # 기본 (28 tests, ~7s, 오프라인)
...
### `tests/unit/` — 순수 함수 (12 tests, ~ms)

외부 의존성 0. input → output.

| 파일 | 검증 대상 |
|------|----------|
| `test_resolve_pvt.py` | `_resolve_pvt` PVT triple 해소 (T1 default, SSPG 모호) |
...
| `test_query_versions.py` | `project_name` / `node` 필터 |
...
**추가 기준**: 사용자가 실제로 묻는 질문 + 기대 동작. 자세한 작성법은 [EvalSet 작성](#evalset-작성-tests-agent) 참조.
