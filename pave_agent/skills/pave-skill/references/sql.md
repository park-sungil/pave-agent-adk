...
| TEMP | VARCHAR2 | 측정 온도. 문자열로 저장 (e.g., "-25", "25", "125") |
| VDD | VARCHAR2 | 공급 전압. 문자열로 저장 (e.g., ".54", ".72", "1") |
...
| CH_TYPE | VARCHAR2 | Cell Height 타입. CH와 1:1 대응 (CH138=uHD, CH168=HD, CH200=HP) |
...
### query_versions
세션 시작 시 1회 전체 로드하여 캐싱. 이후 Python 필터링.
```sql
...
### query_ppa
pdk_id 확정 후 PDK당 전체 데이터를 1회 로드하여 세션에 캐싱. 이후 조건 필터링은 Python에서 수행.
```sql
...
- 여러 행 → 후보 목록 반환, 사용자에게 선택 요청
- 0행 → 에러 + 가능한 옵션 제시

## 주의사항
- PPA 비교 시 반드시 동일 PVT corner (CORNER, VDD, TEMP) 조건 확인
- TEMP, VDD는 DB에서 문자열로 저장됨. 필터링 시 float 비교 fallback 적용
