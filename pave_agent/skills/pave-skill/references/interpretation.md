...
**질문 유형에 따라 포맷을 선택. 사용자가 명시하지 않은 metric이나 분석을 추가하지 말 것.**

### 특정 metric 조회 (예: "주파수가 얼마야?", "누설 전력 알려줘")
- 질문한 **그 metric만** 표로 제시
- 1~2줄 이내 간단 설명 (단위, 조건 등 필요한 경우만)
- trade-off, 다른 metric 언급, 설계 권장사항 **금지**

### 여러 metric 조회 (사용자가 복수 명시)
- 요청한 metric들만 표로 제시
- 필요시 metric 간 관계 간단 언급

### 벤치마킹 비교 (두 PDK 간 delta/% 변화가 포함된 경우)
**Summary**
- 특정 corner 및 조건에서 핵심 변화 요약
