...
`uv run --extra dev pytest -q` 로 27개 deterministic 테스트가 ~9초에 돌아간다 (오프라인, mock SQLite, no LLM).

- `tests/unit/`: 순수 함수 (`_resolve_pvt`, `select_sections`, `deterministic_analysis`)
- `tests/tool/`: 도구 단위 (`query_ppa` needs_input/applied_defaults/state caching, `query_versions` 필터, analyze fast path 가 LLM 호출 안 하는지)
- `tests/conftest.py::FakeToolContext` + `ppa_loaded_state` fixture 가 ADK ToolContext 를 흉내냄

LLM mock 은 `pave_agent.llm.call_llm` 레벨에서 한다 (`litellm.completion` 직접 패치 X — ADK 의 `trace_call_llm` OTel span 이 끊겨서 향후 Langfuse 연동이 깨짐).

Phase 2 (`adk eval` 기반 agent-level), Phase 3 (rubric LLM-as-judge) 는 deferred. 계획: `/Users/sungil/.claude/plans/cuddly-hugging-wadler.md`.
